...
def numpy_to_ros_msg(numpy_array: np.array):
    """ Converts a numpy array used in Open3D to a ROS sensor_msgs/PointCloud.msg """
    pcl_msg = PointCloud()
    pcl_msg.header.stamp = rospy.Time.now()
    pcl_msg.header.frame_id = "cluster"

    for data in numpy_array:
        # Appending because "points" field is an array that takes in Point32
        pcl_msg.points.append(Point32(data[0], data[1], data[2]))
...
def cluster_hdbscan(pcd, is_single_linkage_tree=False):
    ...
    clusterer = hdbscan.HDBSCAN(min_samples=6)
    # Hasprint(dir(clusterer))
    print(dir(clusterer))
    print(f'Min Points: {clusterer.min_samples}')
